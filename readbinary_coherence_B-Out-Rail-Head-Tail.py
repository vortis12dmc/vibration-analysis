...
writeFilePass = "/gdsfs/gdsfs/mukai/jr/analyTrainData/output/B_Out_Rail-Head_Tail/"
...
sensorLimit_top = 2047
sensorLimit_bottom = -2048
...
accTitle1 = "Raw " +sprit_readFile1[7]+'/'+sprit_readFile1[8]+'/'+ axis1 + "-axis" +" Head"
accTitle2 = "Raw " +sprit_readFile2[7]+'/'+sprit_readFile2[8]+'/'+ axis2 + "-axis" + " Tail"
powTitle1 = "Pow " +sprit_readFile1[7]+'/'+sprit_readFile1[8]+'/'+ axis1 + "-axis"
powTitle2 = "Pow " +sprit_readFile2[7]+'/'+sprit_readFile2[8]+'/'+ axis2 + "-axis"
plotTitle = sprit_readFile1[7]+'/'+sprit_readFile1[8]+'/'+ axis1 + "-axis" +' & '+sprit_readFile2[7]+'/'+sprit_readFile2[8]+'/'+ axis2 + "-axis_npseg-"+str(npseg) + " Head-Tail"
...
# signal1のend_indexを求める
# 最初にsensorLimit_topまたはsensorLimit_bottomの値になるデータの1つ前を指定
for i, value in enumerate(num1):
    if value >= sensorLimit_top or value <= sensorLimit_bottom:
        end_index = i - 1
        break
else:
    # センサーの限界値を超えるデータがない場合、リストの末尾を使用
    end_index = len(num1)
...
# 同様の処理
# signal2のstart_indexを求める
# リストの終端から、最初にsensorLimit_topまたはsensorLimit_bottomの値になるデータの1つ後を指定
for i, value in enumerate(reversed(num2)):
    if value >= sensorLimit_top or value <= sensorLimit_bottom:
        start_index = len(num2) - (i + 1)
        break
else:
    # センサーの限界値を超えるデータがない場合、リストの始点を使用
    start_index = 0
end_index = len(num2) - next((i for i, x in enumerate(reversed(num2)) if x >= threshold_val2), len(num2))

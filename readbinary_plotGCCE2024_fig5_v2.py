...
writeFile = writeFilePass+sprit_readFile1[2]+'_'+sprit_readFile1[3]+'_'+ axis1 + "axis" +'__'+sprit_readFile2[2]+'_'+sprit_readFile2[3]+'_'+ axis2 + "axis" + "_coherence_npseg-"+str(npseg)+"_v2.eps"
writeFilePrev = writeFilePass+sprit_readFile1[2]+'_'+sprit_readFile1[3]+'_'+ axis1 + "axis" +'__'+sprit_readFile2[2]+'_'+sprit_readFile2[3]+'_'+ axis2 + "axis" + "_coherence_npseg-"+str(npseg)+"_v2.png"
...
end_index = len(num1) - next((i for i, x in enumerate(reversed(num1)) if x >= threshold_val1), len(num1))
# 条件に合致する部分リストを抽出
...
ax[0].set_ylim(ylim_range_bottom_db, ylim_range_top_db)
ax[0].grid(True)

ax[1].set_title(powTitle2)
ax[1].plot(freq2[:len(freq2)//2], power_in_dB2[:len(power_spectrum2)//2])
ax[1].set_xlabel('Frequency [Hz]')
ax[1].set_ylabel('Power [dB]')
ax[1].set_ylim(ylim_range_bottom_db, ylim_range_top_db)
ax[1].grid(True)


# クロススペクトラムの振幅
ax[2].plot(f, np.abs(Pxy))

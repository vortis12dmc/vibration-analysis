...
ylim_range_top = 105.0
ylim_range_bottom = -105.0

args = sys.argv
arguments_count = len(sys.argv)
if arguments_count < 6:
#    print("Usage: "+args[0]+" <信号1の前半> <信号1の後半> <閾値>")
    print("Usage: "+args[0]+" <信号1の前半> <信号1の後半> <閾値> <開始時間> <終了時間>")
    sys.exit(1)
...
end_time = float(args[5])

# ウィンドウ幅，STFTを施す数の設定 '
t_wndw = 100.0e-3 # 100 milisecond
#t_wndw = 100.0e-2 # 1000 milisecond
n_stft = 50 # number of STFT
...
writeFile = readFile1_1[:-4] + "_STFT_stackUp_timeTrim"+str(start_time)+"-"+str(end_time)+"_"+str(n_stft)+"_100g_"+".eps"
pngWriteFile = readFile1_1[:-4] + "_STFT_stackUp_timeTrim"+str(start_time)+"-"+str(end_time)+"_"+str(n_stft)+"_100g_"+".png"
...
# データが範囲内に収まっているか確認
if start_index < 0 or end_index > len(num1):
    print("Error: Specified time range is out of data bounds.")
    sys.exit(1)
...
print("Time 1: "+ str(len(signal1)/fs) +" s")
...
# sprit_readFile[3]の文字列を置換
modified_string = rename_string(sprit_readFile[3])

# タイトルの設定
fig.suptitle('[' + sprit_readFile[2] + '] [' + modified_string + '] ' + '[' + axis + '] ' + sprit_readFile[4][:-8])
...
ax_sp1 = fig.add_axes([0.1, 0.07, 0.75, 0.35])
cb_sp1 = fig.add_axes([0.9, 0.07, 0.02, 0.35])
...
#スペクトログラムのプロット
ax_sp1.set_xlim(0, end_time-start_time)
...
norm = mpl.colors.Normalize(vmin=-5.5, vmax=6.6)
print("vmin="+str(vmin))
print("vmax="+str(vmax))
cmap = mpl.cm.jet
...
plt.savefig(pngWriteFile, format='png')
plt.close()

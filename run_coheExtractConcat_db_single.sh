...
if [ $# -ne 7 ]; then
    echo "エラー: 引数の数が不正です．"
    echo "使用方法: $0 <ディレクトリ名> <軸> <ディレクトリ名> <軸> <nperseg> <閾値1> <閾値2>"
    exit 1
fi
...
# Pythonスクリプトにファイルを渡す
python3 ./readbinary_coherence_nperseg_extract_concat_db_single.py "$first_file" "$second_file" "$third_file" "$fourth_file" "$nperseg" "$threshold1" "$threshold2"
